...
syn match  shSpecial  "\\[\\\"\'`$]"
" Donovan Rebbechi <elflord@pegasus.rutgers.edu> : added ) to end 
" for embedded echo. Needed for this kind of thing: echo $(echo hello )
...
" Donovan Rebbechi <elflord@pegasus.rutgers.edu> : added matchgroup
" fixes problems with echo inside command Subs. 
if exists("is_kornshell") || exists("is_bash")
syn region  shCommandSub matchgroup=shOperator start="$(" end=")" contains=ALLBUT,shFunction,shCommandSub,shTestOpr,shCase,shEcho
endif
...
if exists("is_bash")
" DOnovan Rebbechi <elflord@pegasus.rutgers.edu> : added shDeref
" fixes problem with declare ${X}=3 etc.
syn region shIdentifier matchgroup=shStatement start="\<\(declare\|typeset\|local\|export\|set\|unset\)\>[^/]"me=e-1 matchgroup=shOperator skip="\\$" end="$\|[;&]" matchgroup=NONE end="#\|="me=e-1 contains=shTestError,shCurlyError,shWrapLineOperator,shDeref
